...
class ApiService: ApiServiceProtocol {
    ...
    func getAllCurrencies() -> Single<CurrenciesResponse> {
        return Single.create { single in
            let task = Task {
                do {
                    let response: CurrenciesResponse = try await self.networkManager.request(endpoint: "/markets?vs_currency=usd&order=market_cap_desc&per_page=100&page=1&sparkline=true")
